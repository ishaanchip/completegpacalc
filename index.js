...
createAccountBtn.addEventListener("click",function(){
  ...
  if (userError==0 && (signInId.value.length==6||signInId.value.length==7) &&signInPassword.value.length>0 && signInName.value.length>0&&signInYear.value.length>0){
    ...
    userStorage.forEach((id)=>{
      if (id.id == signInId.value){
        existUser = true;
        signInWarn.textContent = "User ID already in use!"
      }
    })
  ...
  else if(signInId.value.length<6||signInId.value.length>7){
    signInWarn.textContent = "Enter valid user id";
  }
  else if(signInPassword.value.length===0){
    signInWarn.textContent = "Enter valid password";
  }
  else if(signInName.value.length===0){
    signInWarn.textContent = "Enter valid name";
  }
  else if(signInYear.value.length===0){
    signInWarn.textContent = "Enter valid grade";
  }
  else{
